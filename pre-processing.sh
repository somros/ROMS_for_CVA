...
for YEAR in $(seq $START_YEAR $END_YEAR); do
    ...
    for MONTH in $(seq -w 1 12); do
        MONTHLY_FILE="$INPUT_DIR/b_s_roms_nep_wb_ssp126_moave_${YEAR}_${MONTH}.nc"
        if [ -f "$MONTHLY_FILE" ]; then
            cp "$MONTHLY_FILE" "$TEMP_DIR/"
            FOUND_FILES=$((FOUND_FILES + 1))
            echo "  Found: $(basename $MONTHLY_FILE)"
        else
            echo "  Missing: b_s_roms_nep_wb_ssp126_moave_${YEAR}_${MONTH}.nc"
        fi
    done
    ...
    cdo mergetime $TEMP_DIR/b_s_roms_nep_wb_ssp126_moave_${YEAR}_*.nc $OUTPUT_DIR/annual_${YEAR}.nc
